function getUsernameFromSessionToken() {
...
// === Request Friends ===
function requestFriendsOverSocket() {
...
function requestGameResultsOverSocket(req) {
    return new Promise((resolve, reject) => {
        const socket = window.chatSocket;
        if (!socket || socket.readyState !== WebSocket.OPEN) {
            reject(new Error('Socket not connected'));
...
function renderMatchHistory(results, containerId) {
...
function renderMatchHistory2v2(results, containerId) {
    const container = document.querySelector(`#${containerId} ul`);
    if (!container)
        return;
    container.innerHTML = '';
...
export async function loadFriendsAndMatchHistory(username) {
    try {
