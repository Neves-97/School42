...
ClapTrap::ClapTrap(std::string name) {
    ...
    this ->_energyPoints = 10;
    this->_attackDamage = 0;
    // std::cout << "\e[1;31m ClapTrap ---> Name constructor called for \033[0m" << this->_name << std::endl;
...
void ClapTrap::beRepaired(unsigned int amount) {
    ...
    unsigned int maxRepairable = 10 - _hitPoints;
    if (amount > maxRepairable) {
        std::cout << "ClapTrap ----> " << _name << " cannot be repaired to more than 10 points." << std::endl;
        return;
    }

    _hitPoints += amount;
    _energyPoints--;
    std::cout << "ClapTrap ----> " << _name << " was revitalized " << amount << " points." << std::endl;
}
